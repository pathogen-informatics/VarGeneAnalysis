...
if __name__ == '__main__':
  ...
  for k in xrange(args.min_k, args.max_k):
    ...
    for i in xrange(args.repetitions):
      ...
      training_subsample = all_data.subsample(p=0.7*0.8, remove_features=True)
      classifier = train_classifier(training_subsample, k=k)
      classifiers.append((classifier,training_subsample.feature_names))

    for i,(classifier_a,feature_names_a) in enumerate(classifiers[:-1]):
      for j,(classifier_b,feature_names_b) in enumerate(classifiers[i+1:]):
        logging.debug("Running iteration %s for k=%s" % (i+j, k))
        test_data = all_data.subsample(p=0.2, remove_features=False)
        testing_subsample_a = test_data.sort_feature_names(feature_names_a)
        test_predictions_from_a = predict_cluster(classifier_a,
                                                  testing_subsample_a)
        testing_subsample_b = test_data.sort_feature_names(feature_names_b)
        test_predictions_from_b = predict_cluster(classifier_b,
                                                  testing_subsample_b)
        all_predictions = predict_cluster(classifier_b,
                                          all_data.sort_feature_names(feature_names_b))

        logging.debug("Calculating scores for iteration %s, k=%s" % (i+j, k))
        score = compare_cluster_maps(test_predictions_from_a,
                                     test_predictions_from_b)
        row = [k, "external", score, i, j, args.max_iters, args.cluster_seeds]
        write_row(args.output_file, row)
        scores.append((score, i, j, all_predictions))

    logging.debug("Analysing the best clusters for k=%s" % k)
    # take the predictions in the 80th percentile
    score, i, j, predictions = sorted(scores)[int(args.repetitions*0.8)]
    cluster_string = clusters_to_string(predictions)
    row = [k, "best", score, i, j, args.max_iters, args.cluster_seeds, cluster_string]
    write_row(args.output_file, row)
